  ...
  return (
    <div className="w-full h-screen bg-zinc-300 flex flex-col gap-10 items-center justify-center">
      ...
      <button
        ...
        className="px-2 py-1 bg-purple-700 rounded text-zinc-200"
      >
        Click me
      </button>
      ...
      {songs.map((e, index) => (
        <div key={index} className="song w-80 px-3 py-2 bg-zinc-100 rounded">
          ...
          <p className="text-xs mt-2">{e.lyrics}</p>
          <button
            onMouseOver={handleClickDownload}
            className="px-2 py-1 bg-blue-400 rounded-md mt-3 text-sm text-zinc-100 font-semibold"
          >
            Download now
          </button>
        </div>
